...
export const updateCoupon = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.name) {
    ...
    if (existingCoupon && existingCoupon._id.valueOf() !== couponId) {
...
export const deleteCopon = asyncHandler(async (req, res, next) => {
  const { couponId } = req.params;
  const copon = await couponModel.findById({ _id: couponId });
  if (!copon) {
    return next(new Error("Copon No Found", { cause: 404 }));
  }
